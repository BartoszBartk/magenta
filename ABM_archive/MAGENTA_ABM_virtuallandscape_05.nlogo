...
globals [budget ;; budget used in each period for agri-environmental payments
  landscape ;; information from a raster file (land-use allocation)
  soil-quality ;; information from a raster file (fertility)
  landuse_final ;; raster file to be exported at the end of simulation
]
turtles-own [my-land ;; plots owned by farmer
  total-yield ;; sum of yields of all plots owned by farmer
  income ;; income from yields and payments received (AE payments & net side payments)
]
patches-own [owned-by ;; determines who the plot belongs to
  ascii-val ;; values from landscape raster
  cover ;; whether patch is grassland or river (from asc file)
  ...
  prox-river ;; reports 1 if distance to river is 1
  agg ;; VARIANT 1: reports 1 if plot was within an "agglomeration" (all neighbours with extensive management) in previous period
  ;; VARIANT 2: reports 1 if plot was surrounded by extensive plots in previous period
  ;; VARIANT 3: reports the share of neighbouring plots that were managed extensively in previous period (for agg bonus with different levels)
...
to assign-plots
  ;; assign plots to turtles
  ;; PROBLEM different farm sizes
  ask n-of 200 patches with [cover = "grass"] [
    set owned-by one-of turtles
  ]
  ;; make clear who owns each plot
  ask turtles [
...
to go
  ...
  calc-yield
  ;; 3 variants of agglomeration bonus response; LIMITATION of all three cases: farmers do not distinguish between neighbouring own vs. others' plots
  ;; check-agg-1
  ;; check-agg-2
  check-agg-3
  ask turtles [
...
to calc-pot-profit
  ;; calculate potential profits for each management variant
  ;; calculation for each plot separately
  ask my-land [
    set profit-ext (1 + fertility) ^ 0.5 + base-p + bonus-wat * prox-river + bonus-agg * agg
...
to check-agg-1
  ;; check whether plot is within an "agglomeration" and set variable agg
  ask patches with [manag = "ext"] [
    if all? neighbors [manag = "ext" OR cover = "river"] [
      set agg 1
      ;; same for neighbours (all parts of the agglomeration to be rewarded)
      ask neighbors with [manag = "ext"] [
        set agg 1
      ]
    ]
  ]
...
to check-agg-2
  ;; check whether plot is surrounded by extensive plots
  ask patches with [manag = "ext"] [
    if all? neighbors [manag = "ext" OR cover = "river"] [
      set agg 1
    ]
...
to calc-profit
  ...
  ask my-land [
    ifelse manag = "ext" [
      set profit (1 + fertility) ^ 0.5 + base-p + bonus-wat * prox-river + bonus-agg * agg
    ][
      set profit (2 * (1 + fertility)) ^ 0.5
    ]
  ]
end
...
to R-yield
  ...
  ;; note: yield is normalized here and thus doesn't equal yield calculated by CALC-YIELD
...
@#$#@#$#@
<experiments>
  <experiment name="MAGENTA_experiment_test" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>raster
R-yield
...
R-water</final>
    ...
    <steppedValueSet variable="bonus-agg" first="0" step="0.05" last="1"/>
    <steppedValueSet variable="base-p" first="0" step="0.05" last="1"/>
    <steppedValueSet variable="bonus-wat" first="0" step="0.05" last="1"/>
    <enumeratedValueSet variable="dist">
      <value value="1"/>
      <value value="2"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="MAGENTA_experiment_test_0" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <final>raster
R-yield
