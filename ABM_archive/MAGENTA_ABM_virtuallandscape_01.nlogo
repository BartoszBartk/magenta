...
extensions [gis r]
globals [budget landscape soil-fertility] ;; budget used in each period for agri-environmental payments
...
patches-own [owned-by ;; determines who the plot belongs to
  ...
  agg ;; VARIANT 1: reports 1 if plot was within an "agglomeration" (all neighbours with extensive management) in previous period / VARIANT 2: reports 1 if plot was surrounded by extensive plots in previous period / VARIANT 3: reports the share of neighbouring plots that were managed extensively in previous period (for agg bonus with different levels)
...
to setup
  ...
  set soil-fertility gis:load-dataset "models_data/soil_fertility.asc"
  gis:apply-raster landscape ascii-val
  gis:apply-raster soil-fertility fertility
  ;; define river
  ...
  ;; define grassland
  ask patches with [ascii-val = 1 OR ascii-val = 2] [
    set cover "grass"
  ]
  ...
  ask patches with [cover = "grass"] [
    ...
    set profit-int 0
    set agg 0
    ;; check whether plots are by the river
    ifelse distance min-one-of patches with [cover = "river"] [distance myself] = 1 [
      set prox-river 1 ][
      set prox-river 0
    ]
  ]
...
to calc-pot-profit
  ...
  ask my-land [
    set profit-ext fertility * yield-ext + base-p + bonus-wat * prox-river + bonus-agg * agg
    set profit-int fertility * yield-int
  ]
end
...
to calc-yield
  ...
  ask patches with [manag = "ext"] [
    set yield fertility * yield-ext
  ]
  ask patches with [manag = "int"] [
    set yield fertility * yield-int
  ]
end
...
to calc-profit
  ...
  ask my-land [
    ifelse manag = "ext" [
      set profit fertility * yield-ext + base-p + bonus-wat * prox-river + bonus-agg * agg
    ][
      set profit profit-int
    ]
  ]
end
